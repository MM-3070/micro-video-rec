...
class Cliprec(nn.Module):
    ...
    def forward(self, batch, train_step):
        ...
        #clip_feature = batch['visual_inputs']    #tensor:(batch_size,num_clips,dim) non_overlap clip

        #u_pos_em = self.get_u_pos_interest(batch['uid'])  #tensor:[batch_size,dim]
        #u_neg_em = self.get_u_neg_interest(batch['uid'])  #tensor:[batch_size,dim]
        ...
        #u_en_em = self.get_u_en_interest(batch['uid'])  #tensor:[batch_size,dim]
        ...
        if train_step:
            for i in range(self.num_clips):
                ...
                if i  == 0:
                    ...
                    label = np.hstack((label,a.reshape(-1,1)))    #batch_size*num_clips
            #print(label)
            num_valid_clips = []
            reshape_label = []
            ...
            for j in range(label.shape[0]):
                ...
                if len(index) == 0:  #完播
                    ...
                    num_valid_clips.append(index[0]+1)     #有效clip数 , list , size:batch_size
                    for p in range(int(index[0])):
                        reshape_label.append(torch.tensor(1).unsqueeze(0))  #先添加index个1
            ...
            for k in range(len(num_valid_clips)):
                ...
                if num_valid_clips[k]<2:  #2: 10  3:110  4:1110或1111
                    continue
                ...
                #print((w[np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips:np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips+num_valid_clips[k]-2,:]).size())
            ...
            for j in range(label.shape[0]):
                ...
                if len(index) == 0:  #完播
                    ...
                    num_valid_clips.append(4)     #全部都算有效clip , list , size:batch_size
                    for p in range(self.num_clips):
                        reshape_label.append(torch.tensor(label[j][p]).unsqueeze(0))  #先添加index个1
                        reshape_uid.append(batch['uid'][j].unsqueeze(0))
                    #reshape_label.append(torch.tensor(0).unsqueeze(0))
            reshape_label = torch.cat(reshape_label,dim=0)
        ...
        '''
            ...
            return eva_output   #eval对num_clips个片段得分取平均
        '''
